...
export default function BusinessProfileForm() {
  ...
  const applyDemo = (demoId: keyof typeof BUSINESS_DEMOS) => {
    ...
    // 2. Update Customers
    localStorage.setItem('chatflow_customers', JSON.stringify(demo.customers));

    // 3. Update FAQs
    const formattedFaqs = demo.faqs.map((f, i) => ({
        id: `demo-${demoId}-${i}`,
        question: f.q,
        answer: f.a
    }));
    localStorage.setItem('chatflow_faqs', JSON.stringify(formattedFaqs));

    // 4. Update Messages (Clear old ones)
    const demoMessages: Record<number, any[]> = {};
    demo.customers.forEach(customer => {
        demoMessages[customer.id] = [
            {
                id: Date.now(),
                text: customer.lastMsg,
                type: 'user',
                time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
            }
        ];
    });
    localStorage.setItem('chatflow_messages', JSON.stringify(demoMessages));
